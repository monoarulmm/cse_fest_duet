    ...
    <div class="container mx-auto px-4 py-10">
        <div class="flex justify-between items-center border-b border-cyan-500/20 pb-4 mb-6">
            <h2 class="heading-font text-2xl text-white uppercase">
                {{ $event->name }} <span class="text-cyan-400">Details</span>
            </h2>
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
                ...
                @php
                    $slug = $event->slug;
                    $isIupc = $slug === 'iupc';
                    $isSelectionEvent = in_array($slug, ['ai-hackathon', 'project-showcase']);

                    // শুধুমাত্র নির্দিষ্ট ইভেন্টগুলোর জন্য কার্ডটি দেখাবে
                    $shouldShowTab = $isIupc || $isSelectionEvent;
                @endphp

                @if ($shouldShowTab)
                    @php
                        if ($isIupc) {
                            $route = route('event.slot_list', $slug);
                            $colorClass = 'hover:border-blue-500/50 hover:text-blue-400';
                            // Controller থেকে পাঠানো টোটাল স্লট কাউন্ট
                            $displayCount = $totalSlots ?? 0;
                            $label = 'Slot List';
                        } else {
                            $route = route('event.select_registered', $slug);
                            $colorClass = 'hover:border-purple-500/50 hover:text-purple-400';
                            $displayCount = $counts['selected'] ?? 0;
                            $label = 'Selected';
                        }
                    @endphp
                    ...
                    <a href="{{ $route }}"
                        ...
                        <h4 class="text-3xl font-black text-white transition-colors">{{ $displayCount }}</h4>
